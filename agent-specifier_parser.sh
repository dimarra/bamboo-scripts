...
find $START -name "loadbalancing.xml" |  xargs grep agent-specifier | cut -f2 -d">" | cut -f1 -d"<" | python ${bamboo.scriptdir}/isRegex.py 1> .tmp/agent-specifier_parser.log 2>&1
